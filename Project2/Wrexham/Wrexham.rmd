...
con <- dbConnect(
  RPostgres::Postgres(),
  host = "ddev-wrexham-football-analysis-db",  # DDEV database service name
  port = 5432,
...
closest_competitors %>%
  select(season, division, points, closest_competitor, competitor_points) %>%
  kable(caption = "Wrexham vs Closest Competitor by Season") %>%
  kable_styling()
```
...
for (i in 1:nrow(closest_competitors)) {
  ...
  comparison <- team_season_stats %>%
    filter(
      season == season_info$season,
      division == season_info$division,
      team_name %in% c("Wrexham", competitor_name)
    ) %>%
    arrange(desc(points))
